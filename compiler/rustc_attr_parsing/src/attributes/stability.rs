...
impl<S: Stage> CombineAttributeParser<S> for UnstableRemovedParser {
    ...
    const TEMPLATE: AttributeTemplate =
        template!(List: &[r#"feature = "name", reason = "...", issue = "...", since = "version""#]);
    ...
    fn extend(
        ...
        for param in list.mixed() {
            ...
            let Some(word) = param.path().word() else {
                cx.expected_specific_argument(
                    param.span(),
                    &[sym::feature, sym::reason, sym::issue, sym::since],
                );
                return None;
            };
            match word.name {
                ...
                sym::issue => insert_value_into_option_or_error(cx, &param, &mut issue, word)?,
                _ => {
                    cx.expected_specific_argument(
                        param.span(),
                        &[sym::feature, sym::reason, sym::issue, sym::since],
        ...
        let Some(issue) = issue else {
            cx.missing_name_value(list.span, sym::issue);
            return None;
        };
        ...
        Some(UnstableRemovedFeature { feature, reason, issue, since: version })
    }
}
